...
BASE_DIR  = "/mnt/sde/shaoyang/embedding_data/VindrMammo/example"
CSV_PATH  = os.path.join(BASE_DIR, "vindr_detection_v1_folds.csv")
IMG_DIR   = os.path.join(BASE_DIR, "images_png")
CROP_DIR  = os.path.join(BASE_DIR, "crop")

OUTPUT_TRAIN_JSON = os.path.join(BASE_DIR, "VinDr-Mammo_train.json")
OUTPUT_TEST_JSON  = os.path.join(BASE_DIR, "VinDr-Mammo_test.json")
...
def process_group(args):
    key, before_boxes, class_id, patient_id, split = args
    image_id, class_name = key
    img_path = os.path.join(IMG_DIR, patient_id, image_id + ".png")
...
def main():
    ...
    for row in rows:
        if not row.get("resized_xmin", "").strip():
            continue
        categories = ast.literal_eval(row["finding_categories"])
        if categories == ["No Finding"]:
            continue

        bbox = (
            max(0, int(float(row["resized_xmin"]))),
            max(0, int(float(row["resized_ymin"]))),
            max(0, int(float(row["resized_xmax"]))),
            max(0, int(float(row["resized_ymax"]))),
        )
        image_id = row["image_id"].replace(".png", "")
        patient_id_map[image_id] = row["patient_id"]
        img_split[image_id] = row["split"]

        for cat in categories:
            if cat == "No Finding":
                continue
            groups[(image_id, cat)].append(bbox)
            all_categories.add(cat)

    cat_to_id = {cat: idx for idx, cat in enumerate(sorted(all_categories))}
    print(f"Categories ({len(cat_to_id)}): {cat_to_id}")

    tasks = [
        (key, boxes, cat_to_id[key[1]], patient_id_map[key[0]], img_split[key[0]])
        for key, boxes in groups.items()
    ]
    print(f"Total groups: {len(tasks)}  Workers: {NUM_WORKERS}")
